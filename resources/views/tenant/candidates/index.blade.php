        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="card-title text-uppercase mb-0"> Candidates
                    </h4><br>
                    <h5 class="card-subtitle">All Elections and Candidate Information</h5>
                        ...
                        <table id="config-table" class="table display table-bordered table-striped no-wrap border">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($election->candidates_count > 0)
                                            <span class="badge badge-info">{{ $election->profiles_complete_count }}/{{ $election->candidates_count }}</span>
                                        @else
                                            <span class="badge badge-light">N/A</span>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('candidates.view', $election->id) }}" class="btn btn-sm btn-info text-white mr-1">View</a>
                                        <a href="{{ route('candidates.upload', $election->id) }}" class="btn btn-sm btn-primary text-white mr-1">Upload</a>
                                        @if($election->candidates_count > 0)
                                        <button class="btn btn-sm btn-warning text-white notify-btn" data-url="{{ route('candidates.notify', $election->id) }}" data-election="{{ $election->name }}">Notify</button>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
    ...
    $('.notify-btn').on('click', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#notifyStatus').text(response.message || 'Completed!');

                setTimeout(function() {
                    $('#notifyModal').modal('hide');
                    // Show success alert and reload
                    alert('Success: ' + (response.message || 'Notifications sent successfully!'));
