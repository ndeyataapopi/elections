...
@section('title', 'Upload Candidates')
        ...
        <div class="col-lg-9 col-md-8 col-xs-12 align-self-center">
            <nav aria-label="breadcrumb" class="mt-2 float-md-right float-left">
                <ol class="breadcrumb mb-0 justify-content-end p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Upload Candidates</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="card-title text-uppercase mb-0"> Upload Candidates for {{ $election->name }}
                        <span class="pull-right">
                            <a class="btn btn-dark btn-sm" href="{{ route('candidates.index') }}">
                            ← Back to Candidates </a>
                        </span>
                    </h4><br>
                    <h5 class="card-subtitle">Upload Candidate Information via Excel File</h5>
                    ...
                    <!-- Upload Form -->
                    <form id="uploadForm" enctype="multipart/form-data" method="POST" action="{{ route('candidates.upload.file', $election->id) }}">
                        @csrf
                        <div class="form-group row">
                            ...
                            <div class="col-6">
                                ...
                                <small class="form-text text-muted">Upload candidate data in Excel format (.xlsx or .xls)</small>
                            </div>
                            <div class="col-4">
                                <a href="/templates/candidates-template.xlsx" class="btn btn-outline-info btn-sm" download>
                        ...
                        <div class="form-group col-10 mt-5 row">
                            <button type="submit" class="btn btn-success mr-2" id="uploadBtn">
                                <i class="mdi mdi-upload"></i> Upload Candidates
                            </button>
                            <a href="{{ route('candidates.view', $election->id) }}" class="btn btn-dark">Cancel</a>
                        </div>
                    </form>

                    <!-- Candidates Table (if any exist) -->
                    @if($election->candidates->count() > 0)
                    <hr>
                    <h5 class="card-subtitle">Current Candidates ({{ $election->candidates->count() }})</h5>
            ...
            if (progress >= 100) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        setTimeout(function() {
                            window.location.href = '{{ route("candidates.assign.portfolios", $election->id) }}';
                        }, 1500);
                    },
                    error: function(xhr) {
                        progressText.text('Upload failed!');
                        uploadBtn.prop('disabled', false).html('<i class="mdi mdi-upload"></i> Upload Candidates');
